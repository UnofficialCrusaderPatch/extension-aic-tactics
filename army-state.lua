...
function M.new(native)
  ...
  local function validate(values)
    ...
    for player = 0, 8 do
      ...
      range(values[start + 4], native.game.tribeMemberWords - 1)
      for group = 0, 21 do
  ...
  local function rebuildIndex()
    ...
    for player = 1, 8 do
      local character = core.readInteger((native.game.players + 0x2300) + player * 0x39F4)
      if character >= 2 and character <= 17
        ...
        for group = 0, 21 do
          ...
          if id > 0 and id < 1250 then
            local record = native.game.tribes + id * native.game.tribeStride
