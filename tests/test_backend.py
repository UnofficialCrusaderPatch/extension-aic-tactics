...
def backend():
    ...
    lua.execute('''
      ...
      native={configuration=10000,configurationSize=280,configurationLocked=20000,
        preflight=function()end,activate=function()end}
...
def test_character_storage_commit_and_rollback():
    backend().execute('''
      ...
      assert(memory[10000+5*280]==nil)
      op.rollback()
      for address=base,base+276,4 do assert(memory[address]==0) end
    ''')
...
def test_native_only_updates_keep_existing_loader_lifecycle():
    backend().execute('''
      ...
      prepare(4,{}).commit()
      assert(memory[10000+4*280]==0)
      assert(not pcall(prepare,4,{RecruitPolicy='WeightedRoles'}))
      memory[10000+4*280]=1
