...
def backend():
    ...
    lua.execute('''
      ...
      native={configuration=10000,configurationSize=284,configurationLocked=20000,
...
def test_character_storage_commit_and_rollback():
    backend().execute('''
      ...
      assert(memory[10000+5*284]==nil)
      op.rollback()
      for address=base,base+280,4 do assert(memory[address]==0) end
    ''')
...
def test_composition_is_staged_and_rolled_back_with_the_whole_record():
    backend().execute('''
      ...
      assert(next(memory)==nil)
      op.commit()
      assert(memory[10000+4*284+280]==1)
      op.rollback()
      assert(memory[10000+4*284+280]==0)
...
def test_native_only_updates_keep_existing_loader_lifecycle():
    backend().execute('''
      ...
      prepare(4,{}).commit()
      assert(memory[10000+4*284]==0)
      assert(not pcall(prepare,4,{RecruitPolicy='WeightedRoles'}))
      memory[10000+4*284]=1
