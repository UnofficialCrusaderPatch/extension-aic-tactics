...
static unsigned int reservationOffset;
    ...
    int health = 500, int type = 22, bool allied = false, bool dying = false, int half = 0)
{
    ...
    // Restore the native reservation, including incidental effect queues.
    // The integrated test host has its own read-only code before this range.
    std::memcpy(reinterpret_cast<void*>(ImageBase + reservationOffset), before + reservationOffset, ImageSize - reservationOffset);
