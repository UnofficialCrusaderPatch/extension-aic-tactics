...
try {
    ...
    & "$CompilerRoot/VC/bin/cl.exe" /nologo /W4 /WX /MT /EHsc /O2 /DAIC_RUNTIME_TESTS /D_CRT_SECURE_NO_WARNINGS /Iinclude /Fobuild/ /Febuild/shc141-runtime-tests.exe src/runtime.cpp src/combat.cpp src/army.cpp src/raids.cpp src/integrity.cpp src/recruitment.cpp src/random.cpp src/composition.cpp src/targets.cpp src/incidents.cpp src/shc141_damage.cpp src/shc141_recruitment.cpp src/shc141_groups.cpp tests/shc141_probe_tests.cpp tests/shc141_group_cases.cpp tests/shc141_runtime_cases.cpp tests/shc141_damage_cases.cpp tests/shc141_combat_cases.cpp /link /MANIFEST:NO /BASE:0x400000 /FIXED /MERGE:.text=.aorigin '/SECTION:.aorigin,ERW' /IGNORE:4254 /MERGE:.rdata=.zrdata /MERGE:.data=.zdata kernel32.lib
