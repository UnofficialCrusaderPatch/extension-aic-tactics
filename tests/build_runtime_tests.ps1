...
try {
    ...
    & "$CompilerRoot/VC/bin/cl.exe" /nologo /W4 /WX /MT /EHsc /O2 /DAIC_RUNTIME_TESTS /D_CRT_SECURE_NO_WARNINGS /Iinclude /Fobuild/ /Febuild/shc141-runtime-tests.exe src/runtime.cpp src/recruitment.cpp src/random.cpp src/shc141_recruitment.cpp src/shc141_groups.cpp tests/shc141_probe_tests.cpp tests/shc141_group_cases.cpp tests/shc141_runtime_cases.cpp /link /MANIFEST:NO /BASE:0x400000 /FIXED /MERGE:.text=.ztext /MERGE:.rdata=.zrdata /MERGE:.data=.zdata kernel32.lib
